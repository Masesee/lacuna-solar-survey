...
def predict_counter(args):
    ...
    try:
        with torch.no_grad(), torch.cuda.amp.autocast():
            for batch in tqdm(test_loader, desc="Generating predictions"):
                ...
                # Remove rounding and clamping, keep raw predictions
                panel_pred = panel_pred.squeeze()
                boiler_pred = boiler_pred.squeeze()

                for i in range(len(ids)):
                    # Create two rows per image (one for panels, one for boilers)
                    predictions.extend([
                        {
                            'ID': f"{ids[i]}_pan",
                            'Target': float(panel_pred[i].item())
                        },
                        {
                            'ID': f"{ids[i]}_boil",
                            'Target': float(boiler_pred[i].item())
                        }
                    ])
    ...
    if predictions:
        ...
        # Save raw predictions in competition format with custom filename
        submission_path = os.path.join(args.output_dir, args.submission_name)
        pred_df.to_csv(submission_path, index=False)
        print(f"Submission file saved to {submission_path}")

        # For analysis purposes, create a separate visualization of the distribution
        analysis_df = pd.DataFrame(predictions)
        panel_preds = analysis_df[analysis_df['ID'].str.endswith('_pan')]['Target']
        boiler_preds = analysis_df[analysis_df['ID'].str.endswith('_boil')]['Target']

        plt.figure(figsize=(12, 6))
        plt.subplot(1, 2, 1)
        plt.hist(panel_preds, bins=50)
        plt.xlabel('Panel Count')
        plt.ylabel('Frequency')
        plt.title('Distribution of Predicted Panel Counts')

        plt.subplot(1, 2, 2)
        plt.hist(boiler_preds, bins=50)
